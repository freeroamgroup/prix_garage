        ...
        if ESX.PlayerData.job.name == self.job then
            if Config.UseRadialMenu then
                lib.addRadialItem({{
                    ...
                    icon = self.icon,
                    onSelect = function()
                        TriggerEvent('prix_garage:access-' .. self.name, self)
                    end
                }})
            else
...
function spawnVehicle(vehicleData, plate, coords)
    if not lib.getClosestVehicle(vector3(coords), 5.0, false) then
        ESX.Game.SpawnVehicle(vehicleData.model, vector3(coords), coords.w, function(veh)
            ...
            TriggerServerEvent('prix_garage:setVehicleOut', plate, false)
...
RegisterNetEvent('prix_garage:impoundVehicle', function()
    ...
    if closestVehicle then
        if not IsPedInAnyVehicle(ped) then
            if lib.progressBar({
                ...
                TriggerServerEvent('prix_garage:setVehicleImpound', string.gsub(GetVehicleNumberPlateText(closestVehicle), "%s", ""), true)
...
RegisterNetEvent('prix_garage:takeOutVehicle', function(args)
    DoScreenFadeOut(200)
    Wait(500)
    ExitPreviewMode()
    lib.callback('prix_garage:getVehicle', false, function(vehicle)
        if vehicle then
...
RegisterNetEvent('prix_garage:sendVehicleImpound', function(targetPlate)
    TriggerServerEvent('prix_garage:setVehicleImpound', targetPlate, true)
    lib.notify({
...
RegisterNetEvent('prix_garage:findVehicle', function(targetPlate)
    local vehicles = ESX.Game.GetVehicles()
    local found = false
    for i = 1, #vehicles do
        local vehicle = vehicles[i]
...
RegisterNetEvent('prix_garage:transferVehicle', function(args)
    lib.callback('prix_garage:canPay', false, function(canPay)
        if canPay then
            TriggerServerEvent('prix_garage:setVehicleParking', args.plate, args.zone.parking)
            lib.notify({
...
RegisterNetEvent('prix_garage:access-garage-job', function(zone)
    local options = {}
    ...
    table.insert(options, {
        ...
        arrow = true,
        onSelect = function()
            TriggerEvent('prix_garage:access-garage', zone)
        end
    })
...
function EnterPreviewMode(vehicleData, spawn)
    ...
    lib.callback('prix_garage:setPlayerRoutingBucket', false, function(canContinue)
        if canContinue then
...
RegisterNetEvent('prix_garage:access-garage', function(zone)
    lib.callback('prix_garage:getVehicles', false, function(vehicles)
        local options = {}
        if #vehicles > 0 then
                                ...
                                if v.stored == 1 or v.stored == true then
                                    if v.parking ~= nil and v.parking ~= zone.index then
                                        table.insert(options, {
                                            ...
                                            event = 'prix_garage:transferVehicle'
                                        })
                                    else
                                        ...
                                        table.insert(options, {
                                            ...
                                            event = 'prix_garage:takeOutVehicle'
                                        })
                                    end
                                else
                                    ExitPreviewMode()
                                    table.insert(options, {
                                        ...
                                        icon = 'warehouse',
                                        args = v.plate,
                                        event = 'prix_garage:sendVehicleImpound'
                                    })
...
RegisterNetEvent('prix_garage:access-store', function(zone)
    local ped = PlayerPedId()
    if IsPedInAnyVehicle(ped, false) then
        local currentVehicle = GetVehiclePedIsIn(ped, false)
        if GetPedInVehicleSeat(currentVehicle, -1) == ped then
            local plate = GetVehicleNumberPlateText(currentVehicle)
            lib.callback('prix_garage:checkOwner', false, function(isOwner)
                if isOwner then
                    lib.callback('prix_garage:getVehicle', false, function(vehicle)
                        if vehicle then
                            if zone.job then
                                if zone.job == vehicle.job and zone.type == vehicle.type then
                                    ...
                                    TriggerServerEvent('prix_garage:updateVehicle', plate, vehicleProperties, zone.index, true)
                                    lib.notify({
...
RegisterNetEvent('prix_garage:access-impound', function(zone)
    lib.callback('prix_garage:getImpoundedVehicles', false, function(vehicles)
        local options = {}
                ...
                table.insert(options, {
                    ...
                    onSelect = function()
                        ...
                        table.insert(options, {
                            ...
                            event = 'prix_garage:recoverVehicle'
                        })
                        lib.registerContext({
                            id = 'garage_vehicle_options',
                            title = vehicleTitle,
...
RegisterNetEvent('prix_garage:recoverVehicle', function(args)
    lib.callback('prix_garage:getVehicle', false, function(vehicle)
        if vehicle then
            if vehicle.impound then
                lib.callback('prix_garage:canPay', false, function(canPay)
                    if canPay then
                        ...
                        TriggerServerEvent('prix_garage:setVehicleOut', vehicle.plate, false)
                    else
                        lib.notify({
                            description = locale('not_enought_money'),
                            type = 'error'
...
RegisterNetEvent('prix_garage:buyVehicle', function(args)
    lib.callback('prix_garage:canPay', false, function(canPay)
        if canPay then
            local vehicle = lib.getVehicleProperties(previewVehicle)
            TriggerServerEvent('prix_garage:buyVehicle', vehicle.plate, vehicle, zoneIndex, args.job)
            ExitPreviewMode()
            lib.notify({
